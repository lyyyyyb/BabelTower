...
function preparedSource(text) {
  const messages = openai.buildMessages(text, { targetLanguage: "en", context: "" });
...
assert.strictEqual(openai.preprocessDeadlockTerms("火男来绿路", "en"), "Infernus 来绿路");
...
assert.strictEqual(openai.preprocessDeadlockTerms("猪在绿路", "en"), "MoKrill 在绿路");
...
assert.strictEqual(openai.preprocessDeadlockTerms("球在蓝路", "en"), "Viscous 在蓝路");
assert.strictEqual(openai.preprocessDeadlockTerms("牌没大", "en"), "Wraith 没大");
assert.strictEqual(openai.preprocessDeadlockTerms("猫在紫路", "en"), "Calico 在紫路");
assert.strictEqual(openai.preprocessDeadlockTerms("蛇在黄路", "en"), "Vyper 在黄路");
...
assert.match(systemPrompt("seven no ult dont fight at rift", "zh-Hans"), /rift=裂隙/);
assert.match(systemPrompt("seven no ult dont fight at rift", "zh-Hans"), /official Chinese/);
